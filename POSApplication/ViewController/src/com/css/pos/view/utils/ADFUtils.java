...
public class ADFUtils {
     ...
     */
    public static void updateModel(ValueChangeEvent valueChangeEvent) {
            UIComponent c = valueChangeEvent.getComponent();
            c.processUpdates(FacesContext.getCurrentInstance());
            FacesContext.getCurrentInstance().renderResponse();
    }
    /**
     * get Bound Attribute Label from list of values
     * @param attributeName
     * @return
     */
    public static String getBoundAttributeLabel(String attributeName) {         
            Integer selectedIndex= (Integer) JSFUtils.resolveExpression("#{bindings."+attributeName+".selectedIndex}");  
            return JSFUtils.resolveExpressionAsString("#{bindings."+attributeName+".items["+selectedIndex+"].label}");  
    }
    
    public static  boolean unCommittedDataExist() {
        
           BindingContext bc = BindingContext.getCurrent();
           String dcfName = bc.getCurrentDataControlFrame();
           DataControlFrame dcf = bc.findDataControlFrame(dcfName);
           boolean f = dcf.isTransactionDirty();
           return f;
    }
    
    public static String evaluateReturnAction(){
        String returnAction = null;
        Object currnetModule = JSFUtils.resolveExpression("#{sessionScope.currentModule}");
        
        if(currnetModule!=null){
            if(currnetModule.toString().equalsIgnoreCase("mb")){
                returnAction = "searchMB";
            }else if(currnetModule.toString().equalsIgnoreCase("mp")){
                returnAction = "searchMP";
            }else if(currnetModule.toString().equalsIgnoreCase("flat")){
                returnAction = "searchFlat";
            }
        }
        return returnAction;
    }
